...
A Claude Code plugin that picks the model and reasoning effort for the work in front of a session. It escalates when the session is stuck, stays light for routine work, checks cheap subagents' results, stops "done" without a passing check, and warns when two sessions edit the same file.
...
Or let your agent do it, including the two optional choices (a TypeSafe key for Jev, and which projects may send data to it):

> Install effort-router by following https://github.com/handpickedlab/effort-router/blob/main/INSTALL.md

Needs Claude Code (tested on 2.1.280) and Node 20 or later. Jev is optional: without a TypeSafe API key a built-in heuristic decides, and everything except `rank` works the same.

It works best with a modest default effort (`/effort medium` or `high`). The router raises effort per turn when a task needs it, and the session falls back to your default at the next prompt.
